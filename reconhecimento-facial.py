...
def redim(img, largura):
    ...
    img = cv2.resize(img, (largura, alt), interpolation = cv.INTER_AREA)
    return img
...
camera = cv2.VideoCapture("midias/video.mp4")
...
while True:
    ...
    frame_pb = cv2.cvtColor(frame, cv.COLOR_BGR2GRAY)
    
    #detecta as faces no frame
    faces = df.detectMultiScale(frame_pb, scaleFactor = 1.1,minNeighbors=3, minSize=(20,20), flags=cv.CASCADE_SCALE_IMAGE)
